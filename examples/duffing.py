...
class Duffing(hj.ControlAndDisturbanceAffineDynamics):
    ...
    def __init__(self, alpha=1., beta=1., delta=0., N_Nodes=1, Nodes_u=None, b_u=None, B=None, G=None, ring=False, 
                 control_mode="min",
        ...
        if control_space is None:
            control_space = hj.sets.Box(jnp.array([-alpha, -beta]), 
                                        jnp.array([alpha, beta]))
        if disturbance_space is None:
            disturbance_space = hj.sets.Ball(jnp.zeros(2), delta)
        ...
        if G is None and not(ring):
            self.G = alpha * np.eye(N_Nodes)
        elif ring:
            self.G = alpha * np.eye(N_Nodes) + np.diag(np.ones(N_Nodes - 1), -1) + np.diag(np.ones(N_Nodes - 1), 1) + np.diag([1], N_Nodes-1) + np.diag([1], -N_Nodes+1)
        else:
            self.G = G
    ...
    def f(self, t, x, u, d):
        ...
        xdot[::2]  = x[1::2]
        xdot[1::2] = - self.beta * (x[::2] ** 3) - self.delta * x[1::2] + np.matmul(self.G, x[::2]) + np.matmul(self.B, u)
